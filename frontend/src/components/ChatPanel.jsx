    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.publish({
          ...
          body: JSON.stringify({type:'JOIN', roomId:room.id}),
        })
      },
    })
    client.activate()
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'12px 10px',display:'flex',flexDirection:'column',gap:2}}>
        {messages.map((msg,i) => {
          ...
          const showDate = !prev || !isSameDay(msg.createdAt||msg.timestamp, prev.createdAt||prev.timestamp)
            ...
            <div key={i}>
              {showDate && (
                <div style={{textAlign:'center',fontSize:10,color:'rgba(255,255,255,.35)',margin:'10px 0 6px',letterSpacing:'.03em'}}>
                  {dayjs(msg.createdAt||msg.timestamp).format('YYYY년 M월 D일')}
                </div>
              )}
              ...
              <div style={{display:'flex',justifyContent:isMine?'flex-end':'flex-start',alignItems:'flex-end',gap:4}}>
                ...
                {isMine && (
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:2,marginBottom:1,flexShrink:0}}>
                    ...
                    <span style={{fontSize:9,color:'rgba(255,255,255,.35)',lineHeight:1}}>
                      {dayjs(msg.createdAt||msg.timestamp).format('HH:mm')}
                    </span>
                  </div>
                )}
