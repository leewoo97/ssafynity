  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect:()=>{
        ...
        // 관리자 모니터 추적용 — Redis chat:room:{id}:users 에 등록
        client.publish({destination:'/app/chat.join', body:JSON.stringify({type:'JOIN',roomId:room.id})})
      },
    })
    client.activate()
    ...
    return ()=>{
      if(client.connected) {
        client.publish({destination:'/app/chat.leave', body:JSON.stringify({type:'LEAVE',roomId:room.id})})
      }
      client.deactivate()
    }
  }, [room.id])
